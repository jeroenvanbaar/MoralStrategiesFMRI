function filenamesDicom = GetAllDicomNames_AutoTransfer( FIRST_SERIES, N_SERIES, FOLDER )
% ------------------------------------------------------------------
%
...
for iFirst = nFirst:-1:1
    ...
    for iSeries = currentNSeries:-1:1
        ...
        a = strfind(folderList,sprintf('%03d',currentSeriesNumber));
        subFolder = folderList{(~cellfun(@isempty,a))};
        filesTemp = dir(sprintf('%s/%s/*.IMA',FOLDER,subFolder));
        ...
        a = repmat([FOLDER, '/', subFolder, '/'], size(fileNames,1),1);
        % store filenames
